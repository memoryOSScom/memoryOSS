...
generate_report() {
  ...
  python3 "$ROOT_DIR/tests/generate_report.py" \
    ...
    --update-plane-json "$UPDATE_PLANE_JSON" \
    --universal-loop-json "$UNIVERSAL_LOOP_JSON" \
...
run_extraction_eval_step() {
  ...
  env_args=(
    ...
    -u EXTRACTION_EVAL_PROMPT_PATH
    -u EXTRACTION_EVAL_PROMPT_TEXT
    -u EXTRACTION_EVAL_EXPERIMENTAL_PROVIDER
    -u EXTRACTION_EVAL_EXPERIMENTAL_MODEL
    -u EXTRACTION_EVAL_EXPERIMENTAL_PROMPT_PATH
    -u EXTRACTION_EVAL_EXPERIMENTAL_PROMPT_TEXT
  ...
  if [ -n "${RUN_ALL_EXTRACTION_EVAL_PROMPT_PATH:-}" ]; then
    env_args+=("EXTRACTION_EVAL_PROMPT_PATH=$RUN_ALL_EXTRACTION_EVAL_PROMPT_PATH")
  fi
  if [ -n "${RUN_ALL_EXTRACTION_EVAL_PROMPT_TEXT:-}" ]; then
    env_args+=("EXTRACTION_EVAL_PROMPT_TEXT=$RUN_ALL_EXTRACTION_EVAL_PROMPT_TEXT")
  fi
  ...
  if [ -n "${RUN_ALL_EXTRACTION_EVAL_EXPERIMENTAL_PROVIDER:-}" ]; then
    env_args+=(
      "EXTRACTION_EVAL_EXPERIMENTAL_PROVIDER=$RUN_ALL_EXTRACTION_EVAL_EXPERIMENTAL_PROVIDER"
    )
  fi
  if [ -n "${RUN_ALL_EXTRACTION_EVAL_EXPERIMENTAL_MODEL:-}" ]; then
    env_args+=(
      "EXTRACTION_EVAL_EXPERIMENTAL_MODEL=$RUN_ALL_EXTRACTION_EVAL_EXPERIMENTAL_MODEL"
    )
  fi
  if [ -n "${RUN_ALL_EXTRACTION_EVAL_EXPERIMENTAL_PROMPT_PATH:-}" ]; then
    env_args+=(
      "EXTRACTION_EVAL_EXPERIMENTAL_PROMPT_PATH=$RUN_ALL_EXTRACTION_EVAL_EXPERIMENTAL_PROMPT_PATH"
    )
  fi
  if [ -n "${RUN_ALL_EXTRACTION_EVAL_EXPERIMENTAL_PROMPT_TEXT:-}" ]; then
    env_args+=(
      "EXTRACTION_EVAL_EXPERIMENTAL_PROMPT_TEXT=$RUN_ALL_EXTRACTION_EVAL_EXPERIMENTAL_PROMPT_TEXT"
    )
  fi
